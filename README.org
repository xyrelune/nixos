...
#+end_src
   I only have one home-manager user configured, and that is for my ~laptop~ machine.
...
** Default   
*** Laptop 
#+begin_src nix :tangle modules/hosts/laptop/default.nix 
  {
    withSystem,
    self,
    inputs,
...
** Configuration 
*** Laptop
#+begin_src nix :tangle modules/hosts/laptop/configuration.nix
  {
    self,
    inputs,
    ...
...
** Hardware
*** Laptop
#+begin_src nix :tangle modules/hosts/laptop/hardware.nix 
  {
    self,
    inputs,
    ...
...
** Desktop Environment
*** xfce
#+begin_src nix :tangle modules/features/de/xfce.nix
  {
    self,
    inputs,
    ...
...
#+end_src
*** Graphics
#+begin_src nix :tangle modules/features/misc/graphics.nix
  {
    self,
    inputs,
    ...
...
#+end_src
*** Jellyfin
Added hardware acceleration and transcoding, made /dev/nvidia* accessible to jellyfin for transcoding to actually occur.
https://discourse.nixos.org/t/jellyfin-cuda-nixos-unable-to-transcode/79830/5
...
#+end_src
*** Radicale
#+begin_src nix :tangle modules/features/web/radicale.nix 
  {
    self,
    input,
    ...
...
#+end_src
*** Navidrome
#+begin_src nix :tangle modules/features/web/navidrome.nix
  {
    self,
    input,
    ...
...
#+end_src
*** Ollama
#+begin_src nix :tangle modules/features/web/ollama.nix
  {
    self,
    input,
    ...
...
#+end_src
* Home
- For setting default applications with ~xdg-mime~, run the following command
  example for pdfs
#+begin_src bash :tangle no
  xdg-mime default helium.desktop application/pdf
#+end_src
- For getting the .desktop names for run the following commands
#+begin_src bash :tangle no
  find /run/current-system/sw/share/applications -name '*.desktop'
  find ~/.nix-profile/share/applications -name '*.desktop'
#+end_src
- For setting default web browser use the following
#+begin_src bash :tangle no
...
#+end_src
*** Rofi
#+begin_src nix :tangle modules/home/home-modules/rofi.nix
  {
    self,
    inputs,
    ...
...
One can use nwg-look to check the various names for the available variants, even for the icons.
For icons specifically add hyphens after each word, for example "Gruvbox Plus Dark" need to be entered as "Gruvbox-Plus-Dark".
...
For installing node packages use the following commands for setting the prefix
#+begin_src bash :tangle no
...
#+end_src
** alias
#+begin_src emacs-lisp :tangle modules/home/home-modules/emacs/alias
